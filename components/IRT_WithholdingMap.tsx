...
interface IRTWithholdingMapProps {
    invoices: Invoice[];
    suppliers: Supplier[];
...
const IRTWithholdingMap: React.FC<IRTWithholdingMapProps> = ({ invoices, suppliers, withholdingTypes, onViewInvoice }) => {
    ...
    }, [invoices, suppliers, withholdingTypes, searchTerm, filterYear, filterMonth, irtGrupoBType]);

    const totalWithholding = irtInvoices.reduce((acc: number, inv) => {
        const irtLines = inv.lines.filter(l => l.withholdingTypeId === irtGrupoBType?.id);
        return acc + irtLines.reduce((sum: number, l) => sum + l.withholdingAmount, 0);
    }, 0);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-md transition-all group">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Retido (IRT B)</span>
                    </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-md transition-all group">
                    ...
                    <h3 className="text-3xl font-black text-slate-800">{irtInvoices.length}</h3>
                    <p className="text-xs text-slate-500 mt-2 font-medium">Facturas com retenção IRT B</p>
                </div>

                <div className="bg-blue-600 p-8 rounded-[2.5rem] border border-blue-500 text-white shadow-lg shadow-blue-600/20 group transition-all">
                    <div className="flex items-center justify-between mb-4">
                    ...
                    <h3 className="text-3xl font-black">{irtGrupoBType?.rate || 6.5}%</h3>
                    <p className="text-xs text-white/80 mt-2 font-medium">IRT Grupo B (Serviços)</p>
                </div>
            </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 w-full">
                        <div className="relative flex-1 min-w-[300px]">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none text-sm font-bold text-slate-600"
                            />
                        </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {irtInvoices.map((inv) => {
                                const supplier = suppliers.find(s => s.id === inv.supplierId);
                                const irtLines = inv.lines.filter(l => l.withholdingTypeId === irtGrupoBType?.id);
                                const baseValue = irtLines.reduce((sum, l) => sum + l.taxableValue, 0);
                                const withholdingValue = irtLines.reduce((sum, l) => sum + l.withholdingAmount, 0);
                                ...
                                return (
                                    <tr key={inv.id} className="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            <span className="text-sm font-black text-blue-600">{withholdingValue.toLocaleString('pt-AO', { minimumFractionDigits: 2 })}</span>
                                        </td>
                                        <td className="px-8 py-6">
                            ...
                            {irtInvoices.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-8 py-20 text-center text-slate-400 italic">Nenhuma retenção de IRT Grupo B encontrada.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
...
export default IRTWithholdingMap;
