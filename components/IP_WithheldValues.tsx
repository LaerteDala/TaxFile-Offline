...
interface IPWithheldValuesProps {
    invoices: Invoice[];
    clients: Client[];
    withholdingTypes: WithholdingType[];
}

const IPWithheldValues: React.FC<IPWithheldValuesProps> = ({ invoices, clients, withholdingTypes }) => {
    ...
    const filteredData = useMemo(() => {
        return invoices.filter(inv => {
            ...
            const hasIPWithholding = inv.lines.some(l => l.withholdingTypeId === ipType.id);
            if (!hasIPWithholding) return false;
            ...
            const client = clients.find(c => c.id === inv.clientId);
            const ipWithholding = inv.lines
                .filter(l => l.withholdingTypeId === ipType?.id)
            ...
            return {
                ...
                nif: client?.nif || '---',
                ipAmount: ipWithholding
            };
        });
    }, [invoices, clients, searchTerm, startDate, endDate, ipType]);

    const totalWithheld = useMemo(() => {
        return filteredData.reduce((sum, item) => sum + item.ipAmount, 0);
    }, [filteredData]);
    ...
    return (
        <div className="animate-in fade-in duration-500 space-y-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-slate-800">Valores Retidos - IP</h2>
                    <p className="text-sm text-slate-500 font-medium">Consulta de valores de IP retidos na fonte por clientes</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="bg-blue-600 px-6 py-3 rounded-2xl shadow-lg shadow-blue-600/20">
                        <p className="text-[10px] font-black text-blue-100 uppercase tracking-widest">Total Retido (IP)</p>
                            ...
                            {filteredData.map((row) => (
                                <tr key={row.id} className="hover:bg-blue-50/30 transition-colors group">
                                    ...
                                    <td className="px-8 py-5 text-right">
                                        <span className="text-sm font-black text-blue-600">{row.ipAmount.toLocaleString('pt-AO', { minimumFractionDigits: 2 })}</span>
                                    </td>
                                    <td className="px-8 py-5 text-right">
...
export default IPWithheldValues;
