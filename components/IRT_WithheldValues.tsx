...
interface IRTWithheldValuesProps {
    invoices: Invoice[];
    clients: Client[];
    withholdingTypes: WithholdingType[];
}

const IRT_WithheldValues: React.FC<IRTWithheldValuesProps> = ({ invoices, clients, withholdingTypes }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [startDate, setStartDate] = useState('');
    const [endDate, setEndDate] = useState('');

    const irtType = withholdingTypes.find(wt => wt.name === 'IRT Grupo B');

    const filteredData = useMemo(() => {
        return invoices.filter(inv => {
            if (inv.type !== 'SALE') return false;
            if (!irtType) return false;

            const hasIRTWithholding = inv.lines.some(l => l.withholdingTypeId === irtType.id);
            if (!hasIRTWithholding) return false;
            ...
            const client = clients.find(c => c.id === inv.clientId);
            const irtWithholding = inv.lines
                .filter(l => l.withholdingTypeId === irtType?.id)
            ...
            return {
                ...
                nif: client?.nif || '---',
                irtAmount: irtWithholding
            };
        });
    }, [invoices, clients, searchTerm, startDate, endDate, irtType]);

    const totalWithheld = useMemo(() => {
        return filteredData.reduce((sum, item) => sum + item.irtAmount, 0);
    }, [filteredData]);
    ...
    return (
        <div className="animate-in fade-in duration-500 space-y-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-slate-800">Valores Retidos - IRT</h2>
                    <p className="text-sm text-slate-500 font-medium">Consulta de valores de IRT retidos na fonte por clientes</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="bg-blue-600 px-6 py-3 rounded-2xl shadow-lg shadow-blue-600/20">
                        <p className="text-[10px] font-black text-blue-100 uppercase tracking-widest">Total Retido (IRT)</p>
                            ...
                            {filteredData.map((row) => (
                                <tr key={row.id} className="hover:bg-blue-50/30 transition-colors group">
                                    ...
                                    <td className="px-8 py-5 text-right">
                                        <span className="text-sm font-black text-blue-600">{row.irtAmount.toLocaleString('pt-AO', { minimumFractionDigits: 2 })}</span>
                                    </td>
                                    <td className="px-8 py-5 text-right">
...
export default IRT_WithheldValues;
