...
import { IVAClassification } from '../../types';

interface IVAClassificationsProps {
...
const IVAClassifications: React.FC<IVAClassificationsProps> = ({ ivaClassifications, setIvaClassifications }) => {
    ...
    // Form State
    const [formData, setFormData] = useState<Partial<IVAClassification>>({
        code: '',
        description: '',
        taxableBaseLine: '',
        taxpayerTaxLine: '',
    ...
    const filteredClassifications = (ivaClassifications || []).filter(c => {
    ...
    const handleEdit = (classification: IVAClassification) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const isDuplicate = ivaClassifications.some(c => c.code === formData.code && c.id !== selectedId);
            if (isDuplicate) {
            ...
            if (currentSubView === 'create') {
                ...
                await window.electron.db.addIVAClassification(newClassification);
                setIvaClassifications([...ivaClassifications, newClassification as IVAClassification]);
            } else if (currentSubView === 'edit' && selectedId) {
                const updatedClassification = {
                    ...formData,
                    id: selectedId
                };
                await window.electron.db.updateIVAClassification(updatedClassification);
                setIvaClassifications(ivaClassifications.map(c => c.id === selectedId ? updatedClassification as IVAClassification : c));
            }
    ...
    const removeClassification = async (id: string) => {
        ...
        try {
            await window.electron.db.deleteIVAClassification(id);
            setIvaClassifications(prev => prev.filter(c => c.id !== id));
        } catch (err: any) {
    ...
    const handleSeedDefaults = async () => {
        setIsSubmitting(true);
        try {
            await window.electron.db.seedDefaultIVA();
            const updated = await window.electron.db.getIVAClassifications();
            setIvaClassifications(updated);
            alert('Dados padrão carregados com sucesso!');
        } catch (err: any) {
            alert(`Erro ao carregar dados padrão: ${err.message}`);
        } finally {
            setIsSubmitting(false);
        }
    };
            ...
            <div className="animate-in slide-in-from-right-8 duration-500 max-w-4xl mx-auto pb-12">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <h2 className="text-2xl font-black text-slate-800">
                        {currentSubView === 'create' ? 'Nova Classificação IVA' : 'Editar Classificação IVA'}
                    </h2>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                                ...
                                <div className="md:col-span-3 space-y-2">
                                    ...
                                    <input
                                        required
                                        type="text"
                                        placeholder="Ex: Transmissão de bens..."
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Ex: 1"
                                        value={formData.taxableBaseLine || ''}
                                        onChange={(e) => setFormData({ ...formData, taxableBaseLine: e.target.value })}
                                        className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-slate-800 transition-all"
                                    />
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Ex: 6"
                                        value={formData.taxpayerTaxLine || ''}
                                        onChange={(e) => setFormData({ ...formData, taxpayerTaxLine: e.target.value })}
                                        className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-slate-800 transition-all"
                                    />
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Ex: 7"
                                        value={formData.stateTaxLine || ''}
                                        onChange={(e) => setFormData({ ...formData, stateTaxLine: e.target.value })}
                                        className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-slate-800 transition-all"
                                    />
                                </div>
                            </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-50/80 border-b border-slate-200">
                                ...
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Linhas (B/SP/E)</th>
                                ...
                                <tr key={c.id} className="hover:bg-blue-50/30 transition-colors group">
                                    ...
                                    <td className="px-8 py-5 text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-bold border border-slate-200" title="Linha Base">
                                                {c.taxableBaseLine || '-'}
                                            </span>
                                            <span className="px-2 py-0.5 bg-amber-50 text-amber-600 rounded text-[10px] font-bold border border-amber-100" title="Linha Sujeito Passivo">
                                                {c.taxpayerTaxLine || '-'}
                                            </span>
                                            <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded text-[10px] font-bold border border-emerald-100" title="Linha Estado">
                                                {c.stateTaxLine || '-'}
                                            </span>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colSpan={3} className="px-8 py-24 text-center">
                                        <div className="flex flex-col items-center justify-center text-slate-400">
                                            ...
                                            <p className="text-lg font-bold text-slate-500 mb-4">Nenhuma classificação encontrada</p>
                                            <button
                                                onClick={handleSeedDefaults}
                                                disabled={isSubmitting}
                                                className="px-6 py-3 bg-blue-50 text-blue-600 font-bold rounded-xl hover:bg-blue-100 transition-all flex items-center gap-2"
                                            >
                                                {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <RefreshCw size={18} />}
                                                Reparar Dados Padrão
                                            </button>
                                        </div>
                                    </td>
                                </tr>
...
export default IVAClassifications;
