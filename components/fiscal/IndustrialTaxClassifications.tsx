...
import { IndustrialTaxClassification } from '../../types';

interface IndustrialTaxClassificationsProps {
...
const IndustrialTaxClassifications: React.FC<IndustrialTaxClassificationsProps> = ({ industrialTaxClassifications, setIndustrialTaxClassifications }) => {
    ...
    // Form State
    const [formData, setFormData] = useState<Partial<IndustrialTaxClassification>>({
        code: '',
        description: ''
    });
    const [selectedId, setSelectedId] = useState<string | null>(null);

    const filteredClassifications = (industrialTaxClassifications || []).filter(c => {
    ...
    const handleEdit = (classification: IndustrialTaxClassification) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const isDuplicate = industrialTaxClassifications.some(c => c.code === formData.code && c.id !== selectedId);
            if (isDuplicate) {
            ...
            if (currentSubView === 'create') {
                ...
                await window.electron.db.addIndustrialTaxClassification(newClassification);
                setIndustrialTaxClassifications([...industrialTaxClassifications, newClassification as IndustrialTaxClassification]);
            } else if (currentSubView === 'edit' && selectedId) {
                const updatedClassification = {
                    ...formData,
                    id: selectedId
                };
                await window.electron.db.updateIndustrialTaxClassification(updatedClassification);
                setIndustrialTaxClassifications(industrialTaxClassifications.map(c => c.id === selectedId ? updatedClassification as IndustrialTaxClassification : c));
            }
    ...
    const removeClassification = async (id: string) => {
        ...
        try {
            await window.electron.db.deleteIndustrialTaxClassification(id);
            setIndustrialTaxClassifications(prev => prev.filter(c => c.id !== id));
        } catch (err: any) {
            ...
            <div className="animate-in slide-in-from-right-8 duration-500 max-w-4xl mx-auto pb-12">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <h2 className="text-2xl font-black text-slate-800">
                        {currentSubView === 'create' ? 'Nova Classificação Imposto Industrial' : 'Editar Classificação Imposto Industrial'}
                    </h2>
...
export default IndustrialTaxClassifications;
